...
impl InstanceTracker {
	fn new() -> Self {
		InstanceTracker { use_def: Vec::new() }
	}
	...
	fn add_access(&mut self, access: &Access) {
		...
		if !was_present {
			self.use_def.push((access.statement, match access.category {
	...
	fn calc_dependencies(&self) -> Vec<(DependencyEdge, DependencyType)> {
		...
		let iter = self.use_def
			.iter()
			.zip(self.use_def.iter().skip(1));
