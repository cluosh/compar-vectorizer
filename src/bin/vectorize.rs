...
use vectorizer::{
    dependencies::{LevelDependency, Statement},
    ir, vectorization, };
...
fn main() {
    ...
    let file = match File::create(&format!("{}_vectorized.f90", &project_name)) {
        Ok(f) => f,
        Err(e) => {
            eprintln!(
                "Could not open {}_vectorized.f90 for writing: {}",
                &project_name, e
            );
            return;
        }
    ...
    match vectorization::vectorize(&graph, &ast, writer, false) {
        Ok(_) => (),
        Err(e) => {
